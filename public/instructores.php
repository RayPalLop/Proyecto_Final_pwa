<?php
/**
 * Archivo: instructores.php
 * Ubicación: /public/instructores.php
 * Descripción: Punto de entrada para la gestión de instructores del gimnasio.
 * Requiere autenticación de administrador para acceder.
 * Dirige las solicitudes a las acciones del InstructoresController.
 */

// Incluir el archivo de configuración para la conexión a la base de datos y la gestión de sesión.
require_once '../config/config.php';
// Incluir el controlador de instructores.
require_once '../app/controllers/InstructoresController.php';
...
// Crear una instancia del controlador de instructores, pasándole la conexión PDO.
$controller = new InstructoresController($pdo);

// Determinar la acción a realizar. Por defecto, mostrar la lista de instructores.
...
switch ($action) {
    case 'index':
        $controller->index(); // Mostrar la lista de instructores
        break;
    case 'create':
    ...
    case 'edit':
        if ($id) {
            ...
            $_SESSION['error_message'] = 'ID de instructor no especificado para edición.';
    ...
    case 'delete':
        if ($id) {
            $controller->delete($id); // Eliminar instructor
        } else {
            $_SESSION['error_message'] = 'ID de instructor no especificado para eliminación.';
            header('Location: instructores.php?action=index');
            exit();
        }
        break;
